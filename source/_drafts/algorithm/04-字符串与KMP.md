...
第一种是题目直接给 `list[str]`，比如 `344. 反转字符串` 给的是字符数组 `s`，这时可以原地交换：
...
`541. 反转字符串 II` 给一个字符串 `s` 和整数 `k`，要求每隔 `2k` 个字符，反转前 `k` 个字符；如果剩余字符少于 `k` 个，就全部反转；如果剩余字符在 `k` 到 `2k` 之间，就只反转前 `k` 个。
...
`151. 反转字符串中的单词` 给一个字符串，要求反转单词顺序，并且去掉多余空格。比如：
...
`28. 找出字符串中第一个匹配项的下标` 要求在主串 `haystack` 中找到模式串 `needle` 第一次出现的位置。暴力做法是从主串每个位置开始尝试匹配：
...
`459. 重复的子字符串` 要判断一个字符串能不能由某个子串重复多次构成。比如：
...
- `541. 反转字符串 II`：每 `2k` 一组，只反转前 `k` 个，右边界是 `min(start + k - 1, n - 1)`。
- `151. 反转字符串中的单词`：先处理多余空格，再反转单词顺序；Python 可用 `' '.join(reversed(s.split()))`。
