...
{% mermaid %}
flowchart TB
    R["具体表示"] --> A["构造器 · 选择器<br/>行为契约"]
    A --> S["序列接口<br/>递归数据"]
    S --> M["可变状态<br/>身份 · 消息"]
    M --> O["类 · 实例<br/>属性查找 · 方法绑定"]
    O --> G["对象协议<br/>泛型操作"]
    G --> E["表示选择<br/>算法效率"]
{% endmermaid %}
...
{% mermaid %}
flowchart TB
    H1["改造前：高层运算"] -->|"直接读取 x[0] / x[1]"| T["tuple 与下标布局"]
    T -.引入稳定接口.-> H2["改造后：高层运算"]
    H2 -->|只调用| I["rational · numer · denom"]
    I -->|封装| R["tuple / dict / object"]
{% endmermaid %}

<p class="cp-figure-caption">改变的是高层代码依赖的目标：从易变的存储细节转向稳定的公开接口。</p>
...
{% mermaid %}
flowchart LR
    LIST["list"] --> I["Sequence interface<br/>遍历 · 长度 · 选择"]
    TUPLE["tuple"] --> I
    RANGE["range"] --> I
    LINK["Link"] --> I
    I --> MAP["map"]
...
{% mermaid %}
flowchart TD
    START["观察真实调用方式"] --> NARROW{"调用者是否经常<br/>绕过接口？"}
    NARROW -->|是| WIDEN["接口可能太窄<br/>补充稳定且常用的能力"]
    NARROW -->|否| WIDE{"新实现是否难以自然支持<br/>某些公开操作？"}
    WIDE -->|是| SHRINK["接口可能太宽<br/>移除表示相关承诺"]
    WIDE -->|否| FIT["粒度基本合适<br/>最小但够用"]
    WIDEN --> USE["重新验证用例与契约"]
    SHRINK --> USE
{% endmermaid %}
...
{% mermaid %}
flowchart TB
    DATA["数据抽象<br/>值怎样表示"] --> STATE["可变状态<br/>值怎样随历史演化"]
    STATE --> OBJECT["对象系统<br/>谁保存状态并响应操作"]
{% endmermaid %}
...
{% mermaid %}
flowchart TB
    CHANGE["更换具体表示"] --> SHOULD["应保持稳定<br/>高层算术与业务逻辑"]
    CHANGE --> ALLOWED["允许修改<br/>constructor / selector 实现"]
    CHANGE --> LEAK["被迫修改<br/>直接依赖 tuple、下标或字段布局的代码"]
    LEAK --> PROOF["说明抽象屏障已被穿透"]
{% endmermaid %}
