...
沿基类继续查找
   ↓
若找到普通值：直接返回
若找到函数：与 obj 绑定，返回 bound method
```
...
```text
alice.balance
    └──> 调用 get_balance(alice)
             └──> 读取 alice._balance

alice.balance = 100
    └──> 调用 set_balance(alice, 100)
             └──> 验证后写入 alice._balance
```

因此，真正的数据和访问规则位于不同地方：

```text
alice.__dict__                 Account.__dict__
└── _balance: 100              └── balance: property object
    真正保存数据                    保存读取与写入规则
```
...
| 优先级 | 查找位置 | 例子 |
|---:|---|---|
| 1 | 类及其基类中的 data descriptor | `property` |
| 2 | `obj.__dict__` 中的实例属性 | `self.owner`、`self._balance` |
| 3 | 类及其基类中的 non-data descriptor | 普通实例方法 |
| 4 | 类及其基类中的普通属性 | `interest = 0.02` |
| 5 | `__getattr__` 后备逻辑 | 前面都找不到时再调用 |
...
```text
alice.deposit      → 求值得到 bound method
alice.deposit(20)  → 先取得 bound method，再调用它
```
...
```text
Instance.get：实例 attributes → 所属 Class
Class.get：   类 attributes   → Base Class
```
...
alice.deposit(20)
    ↓
在 alice 的实例属性中查找 deposit
    ↓ 未找到
沿 Account 及其 MRO 查找
    ↓ 找到 Account.deposit 函数
函数的 __get__ 把它与 alice 绑定
    ↓
得到 bound method
    ↓ 调用并传入 20
执行 Account.deposit(alice, 20)
    ↓
读取并修改 alice 自己的 balance
```
